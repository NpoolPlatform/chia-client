// rewrite github.com/cloudflare/circl/sign/bls/bls.go
// 1. add KeyGenFromSKBytes and KeyGenV3
// 2. make it possible to user originally private variables
package bls
...
func (k *PrivateKey[K]) PublicKey() *PublicKey[K] {
	if k.pub == nil {
		...
		case *PrivateKey[G1]:
			kk := any(&k.pub.key).(*G1)
			kk.g.ScalarMult(&k.key, bls.G1Generator())
		case *PrivateKey[G2]:
			kk := any(&k.pub.key).(*G2)
			kk.g.ScalarMult(&k.key, bls.G2Generator())
		default:
			panic(ErrInvalid)
		}
	}
...
func KeyGenV3[K KeyGroup](ikm []byte) (*PrivateKey[K], error) {
	return KeyGen[K](ikm, []byte(BLS12_381_SALT), nil)
}

func KeyGenFromSKBytes[K KeyGroup](okm []byte) (*PrivateKey[K], error) {
	var ss bls.Scalar
	ss.SetBytes(okm)
	if ss.IsZero() == 1 {
		return nil, ErrKeyGen
...
func Verify[K KeyGroup](pub *PublicKey[K], msg []byte, sig Signature) bool {
	...
	case *PublicKey[G1]:
		...
		a, b = aa, bb
		k := any(pub.key).(G1)
		listG1[0], listG1[1] = &k.g, bls.G1Generator()
		listG2[0], listG2[1] = &aa.g, &bb.g
	case *PublicKey[G2]:
		aa, bb := new(G1), new(G1)
		a, b = aa, bb
		k := any(pub.key).(G2)
		listG2[0], listG2[1] = &k.g, bls.G2Generator()
	...
	res := bls.ProdPairFrac(listG1[:], listG2[:], []int{1, -1})
...
func VerifyAggregate[K KeyGroup](pubs []*PublicKey[K], msgs [][]byte, aggSig Signature) bool {
	...
	n := len(pubs)
	listG1 := make([]*bls.G1, n+1)
	listG2 := make([]*bls.G2, n+1)
	listSigns := make([]int, n+1)

	listG1[n] = bls.G1Generator()
	listG2[n] = bls.G2Generator()
	listSigns[n] = -1
	...
	C := bls.ProdPairFrac(listG1, listG2, listSigns)
	return C.IsIdentity()
}
